'''
@File    :   Taobao-10.10.5-Android-群聊收视频.py    
@APPTYPE :   Android
@Model   :   Huawei P50

@Modify Time      @Author    @Version    @Action
------------      -------    --------    -----------
2022/3/19 9:44    xyhu       10.10.5     774
'''
import time
...
def group_rec_video():
    ...
    b_el6.click()
    time.sleep(2)
    b_el7 = b_driver.find_element(By.XPATH, '//android.widget.TextView[@text="短视频"]')
    b_el7.click()
    time.sleep(2)
    b_el8_times = 0
    while b_el8_times < 5:
        try:
            b_el8 = b_driver.find_element(By.ID, 'com.taobao.taobao:id/btn_record')
            b_el8.click()
            time.sleep(20)
            break
        except:
            try:
                ...
                b_el9 = b_driver.find_element(By.XPATH, '//android.widget.Button[@text="允许所有文件"]')  # 第一次需授权
                b_el9.click()
                time.sleep(2)
                b_el8_times += 1
    b_el10 = b_driver.find_element(By.ID, 'com.taobao.taobao:id/btn_record')
    b_el10.click()
    time.sleep(3)
    b_el11 = b_driver.find_element(By.XPATH, '//android.widget.TextView[@text="确定"]')
    b_el11.click()
    time.sleep(7)
