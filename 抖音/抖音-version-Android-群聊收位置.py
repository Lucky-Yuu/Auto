'''
@File    :   抖音-version-Android-群聊收位置.py    
@APPTYPE :   Android
@Model   :   Huawei P50
...
def group_rec_location():
    ...
    b_el5.click()
    time.sleep(2)
    b_el6 = b_driver.find_element(By.XPATH, '//android.widget.TextView[@text="位置"]')
    b_el6.click()
    time.sleep(2)
    b_el7_times = 0
    while b_el7_times < 5:
        try:
            b_el7 = b_driver.find_element(By.XPATH, '//android.widget.TextView[@content-desc="当前位置"]')
            b_el7.click()
            time.sleep(3)
            break
        except:
