'''
@File    :   抖音-version-Android-群聊收语音.py    
@APPTYPE :   Android
@Model   :   Huawei P50
...
def group_rec_voice():
    ...
    while b_el5_times < 5:
        try:
            b_el5 = b_driver.find_element(By.XPATH, '//android.widget.ImageView[@content-desc="语音"]')
            b_el5.click()
            time.sleep(2)
            b_el6 = b_driver.find_element(By.XPATH, '//android.widget.TextView[@text="按住 说话"]')
            TouchAction(b_driver).long_press(b_el6, duration=3000).wait(3000).perform()
            time.sleep(5)
            break
        except:
            b_el7 = b_driver.find_element(By.XPATH, '//android.widget.Button[contains(@text,"允许")]')  # 第一次需授权
            b_el7.click()
            time.sleep(2)
            b_el5_times += 1
