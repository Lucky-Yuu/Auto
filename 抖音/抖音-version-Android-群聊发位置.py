'''
@File    :   抖音-version-Android-群聊发位置.py    
@APPTYPE :   Android
@Model   :   Huawei P50
...
def group_send_location():
    ...
    el5.click()
    time.sleep(2)
    el6 = driver.find_element(By.XPATH, '//android.widget.TextView[@text="位置"]')
    el6.click()
    time.sleep(2)
    el7_times = 0
    while el7_times < 5:
        try:
            el7 = driver.find_element(By.XPATH, '//android.widget.TextView[@content-desc="当前位置"]')
            el7.click()
            time.sleep(3)
            break
        except:
