"""
@File    :   WeChat-version-Android-HUAWEI-群聊收语音.py
@APPTYPE :   Android

@Modify Time      @Author    @Version    @Action
------------      -------    --------    -----------
2022/3/9 17:19    xyhu       8.0.20      772
"""
import time
...
def group_recv_voice():
    ...
    time.sleep(2)
    try:
        a_el3 = a_driver.find_element(By.ACCESSIBILITY_ID, "切换到按住说话")
        a_el3.click()
        time.sleep(3)
        a_el4 = a_driver.find_element(By.ACCESSIBILITY_ID, "按住说话")
        TouchAction(a_driver).long_press(a_el4, duration=3000).wait(3000).perform()
        time.sleep(6)
    except:
        print("当前已切换为按住说话")
        a_el4 = a_driver.find_element(By.ACCESSIBILITY_ID, "按住说话")
